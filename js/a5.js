...
function spinLetter(x, hangingItem) {
    if (!canvas) {
        return;
    }
...
function createHangingLetter(letter) {
    if (!font) {
        return null;
    }
    let options = {
...
function createHangingLetterPicking(hangingThing) {
    ...
    if (stringCopied.children && stringCopied.children[0] && stringCopied.children[0] instanceof THREE.Mesh) {
        letter = stringCopied.children[0];
...
function setHangingLetterPosition(hangingLetter, stringHeight) {
...
function createHangingThing(letter) {
    let hangingLetter = createHangingLetter(letter);
    if (!hangingLetter) {
        return;
    }
    let string = createHangingString();
    let thing = { string: string, letter: hangingLetter, id: hangingThings.length };
...
class GPUPickHelper {
    ...
    pick(cssPosition) {
        ...
        camera.setViewOffset(renderer.getContext().drawingBufferWidth, // full width
        ...
        //read the pixel
        renderer.readRenderTargetPixels(pickingTexture, 0, // x
        0, // y
        1, // width
        1, // height
        pixelBuffer);
...
const GPUPickerHelper = new GPUPickHelper();
requestAnimationFrame(renderScene);
